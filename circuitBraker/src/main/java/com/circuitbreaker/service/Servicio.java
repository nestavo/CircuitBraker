...
import com.circuitbreaker.mock.ApiMock;
...
@Slf4j
@Service
public class Servicio implements InitializingBean {

	private final ApiMock mock;
	private final CircuitBreaker circuitBreaker;

	public Servicio(ApiMock mock, CircuitBreaker circuitBreaker) {
		this.mock = mock;
	...
	public int runApiPostCall() {
		...
		return Decorators.ofSupplier(() -> this.mock.mockPost(6)) // pass a Supplier
				.withCircuitBreaker(this.circuitBreaker) // set CB instance to use
				.withFallback(Arrays.asList(RuntimeException.class), this::fallBackRunApiPostCall) // fallback for
																									// exceptions
